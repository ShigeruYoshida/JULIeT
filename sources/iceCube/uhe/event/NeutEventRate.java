...
*/
	...
	for(ip=0;ip<3;ip++){
       ...
       if(args.length!=3){
            System.out.println(
            "Usage: NeutEventRate logEcascadeThreshold MuEventMatrixName TauEventMatrixName");
            System.exit(0);
        }else{
       ...
       NeutEventMonoEnergyFlux cascadeMuFlux = new NeutEventMonoEnergyFlux(in);
       in.close( );
       in = new DataInputStream(new FileInputStream(eventTauMatrixfileName));
       NeutEventMonoEnergyFlux cascadeTauFlux = new NeutEventMonoEnergyFlux(in);
       in.close( );
