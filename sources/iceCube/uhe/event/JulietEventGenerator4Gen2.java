...
public class JulietEventGenerator4Gen2 {
    ...
    /** Geometry of propagating particle represended by IceCubeGen2Coordinate */
    J3Line particleAxis_J3Line_gen2 = null;  // for IceCube-Gen2
    /** Progation starting point location in IceCube coordinate */
    J3Vector startLocation_J3Vector_ice3 = null;
    /** Progation starting point location in IceCube Gen2 coordinate */
    J3Vector startLocation_J3Vector_gen2 = null;// for IceCube-Gen2
	...
	in IceCubeGen2 cooridinate */
    J3Vector shift_J3Vector_gen2 = null; // for IceCube-Gen2
    ...
    */
	...
	// for IceCube-Gen2
        gen2OuterVol = new Volume(2.2*InjectionGeometryUtils.R_cylinder,
				  2.2*InjectionGeometryUtils.R_cylinder,
				  1.1*InjectionGeometryUtils.z_cylinder);
    ...
    /** Get the point location where the particle run starts.
	Represented by the IceCube Gen3 coordinate */
    public J3Vector wherePrimaryParticleStartsInGen2Coordinate() {// for IceCube-Gen2
	if(startLocation_J3Vector_gen2!=null){
	    return startLocation_J3Vector_gen2;
	}else {
	    return gen2Coordinate.getOrigin();
	}
    }
    ...
    */
	...
	particleAxis_J3Line_gen2 = axis;  // for IceCube-Gen2
    }
    public void definePropagatingParticle(int flavor, int doublet,
					  double energy){
	if(primaryFlavor!=flavor){
    ...
    */
    public void definePropagationGeometry(double x_gen2, double y_gen2, double z_gen2,
					  double nadirAngle_gen2, double azimuthAngle_gen2){
	// for IceCube-Gen2
	...
	J3Vector n_gen2 = gen2Coordinate.getPointVectorFromPolarCoordinate
	    (1.0, Math.toRadians(nadirAngle_gen2), Math.toRadians(azimuthAngle_gen2));
	J3UnitVector direction_J3UnitVector_gen2 = 
	    new J3UnitVector(n_gen2.getX(),n_gen2.getY(),n_gen2.getZ());
	particleAxis_J3Line_gen2 = new J3Line(passingPoint_J3Vector_gen2,
    ...
    */
	...
	J3Line particleAxisIce3 : The primary particle trajectory. Should be
                                  defined by the IceCubeGen2Coordinate.
       </pre>
    */
    public void definePropagationGeometry(J3Line particleAxisGen2){
	// for IceCube-Gen2
	particleAxis_J3Line_gen2 = particleAxisGen2;
    }
    ...
    */
	...
	J3Vector particleEntrance_gen2 =
	    gen2Coordinate.transformVectorToThisCoordinate(particleEntrance,earthCoordinate);
	...
	if(propagationFlag==2){ // starting location is 880 m away 
                                // from the IceCube-Gen2 coordinate origin
	                                        // for IceCube-Gen2
	    double r_earth = point.REarth + gen2Coordinate.getGlacierDepth();
	    J3UnitVector direction_J3UnitVector_gen2 = particleAxis_J3Line_gen2.getDirection();
	    ...
	    double trackLengthMax = 2.0*(point.REarth + gen2Coordinate.getGlacierDepth());
	    double delta_length = 5.0e2; // 5 [m] step
	    boolean foundTheInjectionPoint = false;
	    double distanceMin = Double.POSITIVE_INFINITY;double minimalLength = 0.0;
		...
		if(shift_J3Vector_gen2 != null){
		    distance2primary = 
			J3Vector.subtract(particleAxis_J3Line_gen2,shift_J3Vector_gen2).getLength();
		}
		...
		if(distance2primary<distanceMin){// for IceCube-Gen2
		    distanceMin = distance2primary;
		    minimalLength = particleAxis_J3Line_gen2.getAxisLength();
		}
	    }
	    if(!foundTheInjectionPoint){// outside earth+glacier!!
		//particleAxis_J3Line_gen2.setAxisLength(0.0);
		// back to the earth entrance 
		//System.out.println("not inside the cuboid!");
		particleAxis_J3Line_gen2.setAxisLength(minimalLength);// for IceCube-Gen2
	    }
	    startLocation = particleAxis_J3Line_gen2.getAxisLength();
	...
	// for IceCube-Gen2
	particleAxis_J3Line_gen2.setR0(particleEntrance_gen2);
	particleAxis_J3Line_gen2.setAxisLength(startLocation);
	startLocation_J3Vector_gen2 = new J3Vector(
	...
	//System.out.format("startLocation %f %f %f\n",particleAxis_J3Line_center.getX(),
	//		      particleAxis_J3Line_center.getY(),particleAxis_J3Line_center.getZ());
	//System.out.format("startLocation %f %f %f\n",startLocation_J3Vector_ice3.getX(),
	//		      startLocation_J3Vector_ice3.getY(),startLocation_J3Vector_ice3.getZ());
	...
	particleAxis_J3Line_ice3 = new J3Line(particleEntrance_ice3,
	...
	while(true){
	    ...
	    J3Vector particleLocation_J3Vector_gen2 =
		gen2Coordinate.transformVectorToThisCoordinate(
			       particleAxis_J3Line_center,earthCoordinate);// for IceCube-Gen2
		...
		if((shift_J3Vector_gen2==null &&
		    gen2OuterVol.isJ3LineInsideVolume(particleAxis_J3Line_gen2,
						      axisLengthBefore,
						      axisLengthNow))||
		   (shift_J3Vector_gen2!=null &&
		    gen2OuterVol.isJ3LineInsideVolume(particleAxis_J3Line_gen2,
						      shift_J3Vector_gen2,
						      axisLengthBefore,
						      axisLengthNow))){// for IceCube-Gen2
			haveBeenInsideIce3OuterVolume = 1;
		}
	    }

	    boolean hasExited = false;
	    double REarth = point.REarth + gen2Coordinate.getGlacierDepth();
	    if(particleAxis_J3Line_center.getLength()>REarth){
		hasExited = true;
	    }else{

		if((propagationFlag!=1) ||(haveBeenInsideIce3OuterVolume==1)){// for IceCube-Gen2
		    if(shift_J3Vector_gen2==null){
			if(!gen2OuterVol.isInsideVolume(particleLocation_J3Vector_gen2)) hasExited = true;
		    }else{
			if(!gen2OuterVol.isInsideVolume(particleLocation_J3Vector_gen2,shift_J3Vector_gen2)) 
			    hasExited = true;
		    }
		}
