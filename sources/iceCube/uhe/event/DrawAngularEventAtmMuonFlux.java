...
public class DrawAngularEventAtmMuonFlux {
   ...
   (int )(( Particle.getLogEnergyMinimum()-InteractionsBase.getLogEnergyProducedMinimum())/Particle.getDeltaLogEnergy());
	...
	if(args.length!=2){
            System.out.println("Usage: DrawAnularEventAtmMuonFlux Log(threshold E[GeV]) eventMatrixName");
            System.exit(0);
        }else{
	...
	if(iLogEth<0){
	   ...
	   for(itheta=0;itheta<matrixFileName[upDown].length;itheta++){
		   ...
		   integralFlux[0][index] +=  
		       cascadeFlux.getDFEmgCascadeDLogE(logE,cosTheta)*Particle.getDeltaLogEnergy( );
	       }
		   ...
		   integralFlux[1][index] +=  
		       cascadeFlux.getDFHadronCascadeDLogE(logE,cosTheta)*Particle.getDeltaLogEnergy( );
	       }
		   ...
		   integralFlux[2][index] +=  
		       cascadeFlux.getDFTotalCascadeDLogE(logE,cosTheta)*Particle.getDeltaLogEnergy( );
	       }
