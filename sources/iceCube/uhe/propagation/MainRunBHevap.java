...
public class MainRunBHevap {
	...
	int modelNumber = 0;
	RunPropagationMatrixBHevap run = null;

        if(args.length!=4 && args.length!=5){
            System.out.println(
"Usage: MainRunPropagation NadirAngle[deg] (trajectoryLength[cm]) upDownFlag BH-model-number file-name");
	    System.exit(0);
        }else if(args.length==4){
	    nadirAngle = Double.valueOf(args[0]).doubleValue();
            upDownFlag = Integer.valueOf(args[1]).intValue();
            modelNumber = Integer.valueOf(args[2]).intValue();
            fileName = args[3];
        }else if(args.length==5){
	    nadirAngle = Double.valueOf(args[0]).doubleValue();
	    trajectoryLength = Double.valueOf(args[1]).doubleValue();
            upDownFlag = Integer.valueOf(args[2]).intValue();
            modelNumber = Integer.valueOf(args[3]).intValue();
            fileName = args[4];
	}
	...
	if(upDownFlag ==1){
	    ...
	    mediumNumber = 1; //Rock
	    run =
		new RunPropagationMatrixBHevap(nadirAngle,intSwitch,decaySwitch,
					       mediumNumber,modelNumber);
