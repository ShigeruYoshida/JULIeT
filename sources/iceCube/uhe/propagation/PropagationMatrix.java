...
public class PropagationMatrix {
    ...
    private static String grLeptonMtxObjectFile = "eGlashowResonanceLeptonicMtx";
    ...
    private static String grHadronMtxObjectFile = "glashowResonanceHadronicMtx";
    ...
    public PropagationMatrix(Particle nuE, Particle nuMu, Particle nuTau,
			     ...
			     String grLeptonMtxObjectFile_, String grHadronMtxObjectFile_) 
    throws IOException{
	...
	if((interactionsSwitch & GR_FLAG) == GR_FLAG){
	    String objectFile = pathName[s.getMaterialNumber()].concat(grLeptonMtxObjectFile_);
	    InputStream in = ClassLoader.getSystemResourceAsStream(objectFile);
	    grLeptonMtx = InteractionsMatrixInput.inputInteractionsMatrix(in);
	    in.close( );
	    objectFile = pathName[s.getMaterialNumber()].concat(grHadronMtxObjectFile_);
	    in = ClassLoader.getSystemResourceAsStream(objectFile);
	...
	this(nuE, nuMu, nuTau, e, mu, tau, pi, s,interactionsSwitch, decaySwitch, neutrinoFactor,
	     nuCCMtxObjectCCH5File,nuNCMtxObjectCCH5File,grLeptonMtxObjectFile, grHadronMtxObjectFile);
    }
	...
	this(nuE, nuMu, nuTau, e, mu, tau, pi, s,interactionsSwitch, decaySwitch, 1.0,
	     nuCCMtxObjectFile, nuNCMtxObjectFile, grLeptonMtxObjectFile, grHadronMtxObjectFile);
    }

    /** Constructor. Reading all the InteractiosMatrix objects
	and generating the DecayMatrix objects. The infinitesimal propagation 
	distance dX [g/cm^2] is also determined here. The neutrino-nuclen and glashow interaction files
	are given in the arguments. The default neutrino factor*/
    public PropagationMatrix(Particle nuE, Particle nuMu, Particle nuTau,
			     Particle e,   Particle mu,   Particle tau,
			     Particle pi,  ParticlePoint s,
			     int interactionsSwitch, int decaySwitch,
			     String nuCCMtxObjectFile, String nuNCMtxObjectFile,
			     String grLeptonMtxObjectFile_, String grHadronMtxObjectFile_) throws IOException{
	this(nuE, nuMu, nuTau, e, mu, tau, pi, s,interactionsSwitch, decaySwitch, 1.0,
	     nuCCMtxObjectFile, nuNCMtxObjectFile, grLeptonMtxObjectFile_, grLeptonMtxObjectFile_);
    }
