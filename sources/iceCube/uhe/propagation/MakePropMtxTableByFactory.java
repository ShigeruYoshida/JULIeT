...
public class MakePropMtxTableByFactory {
	...
	double binWidth = 0.02; // 0.02 decade 
        if(args.length<3){
            System.out.println(
	...
	while(logInIceEnergy < logInIceMaxEnergy){// loop over inice particle
		...
		double fluxToThisInIceEnergy = 0.0;
		...
		while(cummulativeInIceDeltaLogE < binWidth){
			...
			// from nu-e
			flux += matrix.getDF(0,0,logSurfaceEnergy+cummulativeDeltaLogE,
					     iniceFlavor,iniceDoublet,
					     logInIceEnergy+cummulativeInIceDeltaLogE);
			// from nu-mu
			flux += matrix.getDF(1,0,logSurfaceEnergy+cummulativeDeltaLogE,
					     iniceFlavor,iniceDoublet,
					     logInIceEnergy+cummulativeInIceDeltaLogE);
			// from nu-tau
			flux += matrix.getDF(2,0,logSurfaceEnergy+cummulativeDeltaLogE,
					     iniceFlavor,iniceDoublet,
					     logInIceEnergy+cummulativeInIceDeltaLogE);
