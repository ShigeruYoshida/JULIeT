...
public class NeutrinoNeutralZeusDemo {
	...
	NeutrinoNeutralZeusNewPDF muNC = new NeutrinoNeutralZeusNewPDF(mu, s);
	System.out.println(muNC.interactionName( ));
	...
	for(iLogE=0;iLogE<=500;iLogE+=100){
	    muNC.setIncidentParticleEnergy(iLogE);
	    System.out.println("The Incident energy " + 
			   muNC.getIncidentParticleEnergy() + " GeV");
	    ...
	    // Inelasticity
	    yAverage = muNC.getYDSigmaDy(muNC.getYmin(),muNC.getYmax())/sigma;
	    System.out.println("  Inelasticity <y> " + yAverage);
	    zAverage =  muNC.getZDSigmaDZ(1.0-muNC.getYmax(),
					       1.0-muNC.getYmin())/sigma;
