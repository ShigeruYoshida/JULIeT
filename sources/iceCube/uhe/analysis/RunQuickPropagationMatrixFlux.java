...
Written by S.Yoshida 2007 Feb 15th
*/

public class RunQuickPropagationMatrixFlux {
	...
	if(args.length!=3){
            System.out.println("Usage: RunQuickPropagationMatrixFlux numEvents nuCCenhance calTheory(1 true)");
            System.exit(0);
	}else{
	    numberOfEvents = Double.valueOf(args[0]).doubleValue();
	    nuCCEnhancementFactor = Double.valueOf(args[1]).doubleValue();
	    if(Integer.valueOf(args[2]).intValue()==1) calTheory = true;
	}
	...
	while(logNeutrinoEnergy < logNeutrinoMaxEnergy){ // E < 10^12 GeV
	    double flux = iceFlux.getDFDLogE(logNeutrinoEnergy,numberOfEvents);
	    if(flux>0.0){
